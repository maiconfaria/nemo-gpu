...
INTEGER ::  pos
      ...
      !                     !==  drag nature  ==!
      !
      pos=index(numnam_ref,"&namdrg") 
      READ  ( numnam_ref(pos:), namdrg, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam( ios , 'namdrg in reference namelist' )
      pos=index(numnam_cfg,"&namdrg") 
      READ  ( numnam_cfg(pos:), namdrg, IOSTAT = ios, ERR = 902 )
...
INTEGER ::  pos
      ...
      !                          !==  read namlist  ==!
      !
      pos=index(numnam_ref,"&namdrg_top") 
      IF(ll_top)   READ  ( numnam_ref(pos:), namdrg_top, IOSTAT = ios, ERR = 901)
      pos=index(numnam_ref,"&namdrg_bot") 
      IF(ll_bot)   READ  ( numnam_ref(pos:), namdrg_bot, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam( ios , TRIM(cl_namref) )
      pos=index(numnam_cfg,"&namdrg_top") 
      IF(ll_top)   READ  ( numnam_cfg(pos:), namdrg_top, IOSTAT = ios, ERR = 902 )
      pos=index(numnam_cfg,"&namdrg_bot") 
      IF(ll_bot)   READ  ( numnam_cfg(pos:), namdrg_bot, IOSTAT = ios, ERR = 902 )
