...
INTEGER ::  pos
      ...
      !!----------------------------------------------------------------------
      !
      pos=index(numnam_ref,"&namzdf_tke") 
      READ  ( numnam_ref(pos:), namzdf_tke, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namzdf_tke in reference namelist' )

      pos=index(numnam_cfg,"&namzdf_tke") 
      READ  ( numnam_cfg(pos:), namzdf_tke, IOSTAT = ios, ERR = 902 )
