...
INTEGER ::  pos
      ...
      !!----------------------------------------------------------
      !
      pos=index(numnam_ref,"&namzdf_gls") 
      READ  ( numnam_ref(pos:), namzdf_gls, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namzdf_gls in reference namelist' )

      pos=index(numnam_cfg,"&namzdf_gls") 
      READ  ( numnam_cfg(pos:), namzdf_gls, IOSTAT = ios, ERR = 902 )
