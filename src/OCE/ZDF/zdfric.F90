...
INTEGER ::  pos
      ...
      !!----------------------------------------------------------------------
      !
      pos=index(numnam_ref,"&namzdf_ric") 
      READ  ( numnam_ref(pos:), namzdf_ric, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namzdf_ric in reference namelist' )

      pos=index(numnam_cfg,"&namzdf_ric") 
      READ  ( numnam_cfg(pos:), namzdf_ric, IOSTAT = ios, ERR = 902 )
