...
INTEGER ::  pos
     ...
     !!----------------------------------------------------------------------
     !
      pos=index(numnam_ref,"&namzdf_osm") 
     READ  ( numnam_ref(pos:), namzdf_osm, IOSTAT = ios, ERR = 901)
901  IF( ios /= 0 ) CALL ctl_nam ( ios , 'namzdf_osm in reference namelist' )

      pos=index(numnam_cfg,"&namzdf_osm") 
     READ  ( numnam_cfg(pos:), namzdf_osm, IOSTAT = ios, ERR = 902 )
