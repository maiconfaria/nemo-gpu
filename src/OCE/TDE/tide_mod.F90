...
INTEGER ::  pos
      ...
      pos=index(numnam_ref,"&nam_tide") 
      READ  ( numnam_ref(pos:), nam_tide, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'nam_tide in reference namelist' )
      !
      pos=index(numnam_cfg,"&nam_tide") 
      READ  ( numnam_cfg(pos:), nam_tide, IOSTAT = ios, ERR = 902 )
