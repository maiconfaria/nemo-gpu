...
CONTAINS
   ...
   SUBROUTINE dyn_vor_init
      ...
      ENDIF
      !
      pos=index(numnam_ref,"&namdyn_vor") 
      READ  ( numnam_ref(pos:), namdyn_vor, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namdyn_vor in reference namelist' )
      pos=index(numnam_cfg,"&namdyn_vor") 
      READ  ( numnam_cfg(pos:), namdyn_vor, IOSTAT = ios, ERR = 902 )
