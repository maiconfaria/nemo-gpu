...
INTEGER ::  pos
      ...
      pos=index(numnam_ref,"&namtra_mle") 
      READ  ( numnam_ref(pos:), namtra_mle, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namtra_mle in reference namelist' )

      pos=index(numnam_cfg,"&namtra_mle") 
      READ  ( numnam_cfg(pos:), namtra_mle, IOSTAT = ios, ERR = 902 )
