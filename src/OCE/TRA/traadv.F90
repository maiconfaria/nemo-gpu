...
!!gm ???
   ...
   SUBROUTINE tra_adv_init
      ...
      !
      !                                !==  Namelist  ==!
      pos=index(numnam_ref,"&namtra_adv") 
      READ  ( numnam_ref(pos:), namtra_adv, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namtra_adv in reference namelist' )
      !
      pos=index(numnam_cfg,"&namtra_adv") 
      READ  ( numnam_cfg(pos:), namtra_adv, IOSTAT = ios, ERR = 902 )
