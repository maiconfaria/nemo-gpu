...
INTEGER ::  pos
      ...
      !!----------------------------------------------------------------------
      !
      pos=index(numnam_ref,"&namtrd_mxl") 
      READ  ( numnam_ref(pos:), namtrd_mxl, IOSTAT = ios, ERR = 901 )
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namtrd_mxl in reference namelist' )

      pos=index(numnam_cfg,"&namtrd_mxl") 
      READ  ( numnam_cfg(pos:), namtrd_mxl, IOSTAT = ios, ERR = 902 )
