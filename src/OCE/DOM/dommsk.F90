...
INTEGER ::  pos
      ...
      !!---------------------------------------------------------------------
      !
      pos=index(numnam_ref,"&namlbc") 
      READ  ( numnam_ref(pos:), namlbc, IOSTAT = ios, ERR = 901 )
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namlbc in reference namelist' )
      pos=index(numnam_cfg,"&namlbc") 
      READ  ( numnam_cfg(pos:), namlbc, IOSTAT = ios, ERR = 902 )
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namlbc in configuration namelist' )
      ...
      pos=index(numnam_ref,"&nambdy") 
      READ  ( numnam_ref(pos:), nambdy, IOSTAT = ios, ERR = 903)
903   IF( ios /= 0 )   CALL ctl_nam ( ios , 'nambdy in reference namelist' )
      pos=index(numnam_cfg,"&nambdy") 
      READ  ( numnam_cfg(pos:), nambdy, IOSTAT = ios, ERR = 904 )
