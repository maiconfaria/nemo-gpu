...
INTEGER ::  pos
      ...
      pos=index(numnam_ref,"&namc1d_uvd") 
      READ  ( numnam_ref(pos:), namc1d_uvd, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namc1d_uvd in reference namelist' )
      !
      pos=index(numnam_cfg,"&namc1d_uvd") 
      READ  ( numnam_cfg(pos:), namc1d_uvd, IOSTAT = ios, ERR = 902 )
