...
CONTAINS
   ...
   SUBROUTINE dia_hsb( kt, Kbb, Kmm )
      ...
      !!!
      print *, "zdiff_v2 , kt cpu", zdiff_v2 , kt
      !!!
...
!!gm end
   ...
   SUBROUTINE dia_hsb_init( Kmm )
      ...
      IF(lwp) THEN
         ...
         WRITE(numout,*) '~~~~~~~~~~~~ '
      ENDIF
      pos=index(numnam_ref,"&namhsb") 
      READ  ( numnam_ref(pos:), namhsb, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namhsb in reference namelist' )
      pos=index(numnam_cfg,"&namhsb") 
      READ  ( numnam_cfg(pos:), namhsb, IOSTAT = ios, ERR = 902 )
