...
INTEGER ::  pos
      ...
      pos=index(numnam_ref,"&nam_diadct") 
     READ  ( numnam_ref(pos:), nam_diadct, IOSTAT = ios, ERR = 901)
901  IF( ios /= 0 ) CALL ctl_nam ( ios , 'nam_diadct in reference namelist' )

      pos=index(numnam_cfg,"&nam_diadct") 
     READ  ( numnam_cfg(pos:), nam_diadct, IOSTAT = ios, ERR = 902 )
