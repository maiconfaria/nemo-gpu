...
INTEGER ::  pos
      ...
      !!----------------------------------------------------------------------
      !
      pos=index(numnam_ref,"&nam_dia25h") 
      READ   ( numnam_ref(pos:), nam_dia25h, IOSTAT=ios, ERR= 901 )
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'nam_dia25h in reference namelist' )
      pos=index(numnam_cfg,"&nam_dia25h") 
      READ  ( numnam_cfg(pos:), nam_dia25h, IOSTAT = ios, ERR = 902 )
