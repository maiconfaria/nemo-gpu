...
CONTAINS
   ...
   SUBROUTINE ldf_tra_init
      ...
      ! =================================
      !
      pos=index(numnam_ref,"&namtra_ldf") 
      READ  ( numnam_ref(pos:), namtra_ldf, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namtra_ldf in reference namelist' )
      pos=index(numnam_cfg,"&namtra_ldf") 
      READ  ( numnam_cfg(pos:), namtra_ldf, IOSTAT = ios, ERR = 902 )
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namtra_ldf in configuration namelist' )
   ...
   SUBROUTINE ldf_eiv_init
      ...
      ENDIF
      !
      pos=index(numnam_ref,"&namtra_eiv") 
      READ  ( numnam_ref(pos:), namtra_eiv, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namtra_eiv in reference namelist' )
      !
      pos=index(numnam_cfg,"&namtra_eiv") 
      READ  ( numnam_cfg(pos:), namtra_eiv, IOSTAT = ios, ERR = 902 )
