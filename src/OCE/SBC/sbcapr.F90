...
INTEGER ::  pos
      ...
      !!----------------------------------------------------------------------
      pos=index(numnam_ref,"&namsbc_apr") 
      READ  ( numnam_ref(pos:), namsbc_apr, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namsbc_apr in reference namelist' )

      pos=index(numnam_cfg,"&namsbc_apr") 
      READ  ( numnam_cfg(pos:), namsbc_apr, IOSTAT = ios, ERR = 902 )
