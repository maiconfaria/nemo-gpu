...
CONTAINS
   ...
   SUBROUTINE sbc_wave_init
      ...
      !!---------------------------------------------------------------------
      !
      pos=index(numnam_ref,"&namsbc_wave") 
      READ  ( numnam_ref(pos:), namsbc_wave, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namsbc_wave in reference namelist' )
         
      pos=index(numnam_cfg,"&namsbc_wave") 
      READ  ( numnam_cfg(pos:), namsbc_wave, IOSTAT = ios, ERR = 902 )
