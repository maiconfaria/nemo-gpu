...
INTEGER ::  pos
      ...
      pos=index(numnam_ref,"&namsbc_flx") 
         READ  ( numnam_ref(pos:), namsbc_flx, IOSTAT = ios, ERR = 901)
901      IF( ios /= 0 )   CALL ctl_nam ( ios , 'namsbc_flx in reference namelist' )

      pos=index(numnam_cfg,"&namsbc_flx") 
         READ  ( numnam_cfg(pos:), namsbc_flx, IOSTAT = ios, ERR = 902 )
