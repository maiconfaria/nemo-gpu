...
INTEGER ::  pos
      ...
      ENDIF
      ! 
      pos=index(numnam_ref,"&namsbc_ssr") 
      READ  ( numnam_ref(pos:), namsbc_ssr, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namsbc_ssr in reference namelist' )

      pos=index(numnam_cfg,"&namsbc_ssr") 
      READ  ( numnam_cfg(pos:), namsbc_ssr, IOSTAT = ios, ERR = 902 )
