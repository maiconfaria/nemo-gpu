...
INTEGER ::  pos
      ...
      pos=index(numnam_ref,"&namsbc_iif") 
         READ  ( numnam_ref(pos:), namsbc_iif, IOSTAT = ios, ERR = 901)
901      IF( ios /= 0 )   CALL ctl_nam ( ios , 'namsbc_iif in reference namelist' )

      pos=index(numnam_cfg,"&namsbc_iif") 
         READ  ( numnam_cfg(pos:), namsbc_iif, IOSTAT = ios, ERR = 902 )
