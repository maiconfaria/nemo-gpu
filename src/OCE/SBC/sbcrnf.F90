...
INTEGER ::  pos
      ...
      !                                   ! ============
      !
      pos=index(numnam_ref,"&namsbc_rnf") 
      READ  ( numnam_ref(pos:), namsbc_rnf, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namsbc_rnf in reference namelist' )

      pos=index(numnam_cfg,"&namsbc_rnf") 
      READ  ( numnam_cfg(pos:), namsbc_rnf, IOSTAT = ios, ERR = 902 )
