...
INTEGER ::  pos
      ...
      !
      !                             !** read bulk namelist
      pos=index(numnam_ref,"&namsbc_blk") 
      READ  ( numnam_ref(pos:), namsbc_blk, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namsbc_blk in reference namelist' )
      !
      pos=index(numnam_cfg,"&namsbc_blk") 
      READ  ( numnam_cfg(pos:), namsbc_blk, IOSTAT = ios, ERR = 902 )
