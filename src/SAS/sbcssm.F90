...
INTEGER ::  pos
      ...
      ENDIF
      !
      pos=index(numnam_ref,"&namsbc_sas") 
      READ  ( numnam_ref(pos:), namsbc_sas, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namsbc_sas in reference namelist' )
      pos=index(numnam_cfg,"&namsbc_sas") 
      READ  ( numnam_cfg(pos:), namsbc_sas, IOSTAT = ios, ERR = 902 )
