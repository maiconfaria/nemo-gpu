...
INTEGER ::  pos
      ...
      !!------------------------------------------------------------------
      !
      pos=index(numnam_ice_ref,"&namitd") 
      READ  ( numnam_ice_ref(pos:), namitd, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namitd in reference namelist' )
      pos=index(numnam_ice_cfg,"&namitd") 
      READ  ( numnam_ice_cfg(pos:), namitd, IOSTAT = ios, ERR = 902 )
