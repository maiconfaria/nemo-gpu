...
INTEGER ::  pos
      ...
      !!-------------------------------------------------------------------
      !
      pos=index(numnam_ice_ref,"&namsbc") 
      READ  ( numnam_ice_ref(pos:), namsbc, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namsbc in reference namelist' )
      pos=index(numnam_ice_cfg,"&namsbc") 
      READ  ( numnam_ice_cfg(pos:), namsbc, IOSTAT = ios, ERR = 902 )
