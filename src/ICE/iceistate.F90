...
INTEGER ::  pos
      ...
      !!-----------------------------------------------------------------------------
      !
      pos=index(numnam_ice_ref,"&namini") 
      READ  ( numnam_ice_ref(pos:), namini, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namini in reference namelist' )
      pos=index(numnam_ice_cfg,"&namini") 
      READ  ( numnam_ice_cfg(pos:), namini, IOSTAT = ios, ERR = 902 )
