...
INTEGER ::  pos
      ...
      !!-------------------------------------------------------------------
      !
      pos=index(numnam_ice_ref,"&namthd") 
      READ  ( numnam_ice_ref(pos:), namthd, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namthd in reference namelist' )
      pos=index(numnam_ice_cfg,"&namthd") 
      READ  ( numnam_ice_cfg(pos:), namthd, IOSTAT = ios, ERR = 902 )
