...
INTEGER ::  pos
      ...
      !!-------------------------------------------------------------------
      !
      pos=index(numnam_ice_ref,"&namdyn") 
      READ  ( numnam_ice_ref(pos:), namdyn, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namdyn in reference namelist' )
      pos=index(numnam_ice_cfg,"&namdyn") 
      READ  ( numnam_ice_cfg(pos:), namdyn, IOSTAT = ios, ERR = 902 )
