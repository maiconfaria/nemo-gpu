...
INTEGER ::  pos
      ...
      !!----------------------------------------------------------------------
      !
      pos=index(numnam_ice_ref,"&namalb") 
      READ  ( numnam_ice_ref(pos:), namalb, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namalb in reference namelist' )
      pos=index(numnam_ice_cfg,"&namalb") 
      READ  ( numnam_ice_cfg(pos:), namalb, IOSTAT = ios, ERR = 902 )
