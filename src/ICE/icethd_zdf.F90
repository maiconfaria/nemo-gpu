...
INTEGER ::  pos
      ...
      !!-------------------------------------------------------------------
      !
      pos=index(numnam_ice_ref,"&namthd_zdf") 
      READ  ( numnam_ice_ref(pos:), namthd_zdf, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namthd_zdf in reference namelist' )
      pos=index(numnam_ice_cfg,"&namthd_zdf") 
      READ  ( numnam_ice_cfg(pos:), namthd_zdf, IOSTAT = ios, ERR = 902 )
