...
INTEGER ::  pos
      ...
      !!-------------------------------------------------------------------
      !
      pos=index(numnam_ice_ref,"&nampar") 
      READ  ( numnam_ice_ref(pos:), nampar, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'nampar in reference namelist' )
      pos=index(numnam_ice_cfg,"&nampar") 
      READ  ( numnam_ice_cfg(pos:), nampar, IOSTAT = ios, ERR = 902 )
