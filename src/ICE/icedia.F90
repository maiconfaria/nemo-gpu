...
INTEGER ::  pos
      ...
      !!----------------------------------------------------------------------
      !
      pos=index(numnam_ice_ref,"&namdia") 
      READ  ( numnam_ice_ref(pos:), namdia, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namdia in reference namelist' )
      pos=index(numnam_ice_cfg,"&namdia") 
      READ  ( numnam_ice_cfg(pos:), namdia, IOSTAT = ios, ERR = 902 )
