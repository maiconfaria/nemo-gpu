...
INTEGER ::  pos
      ...
      !!-------------------------------------------------------------------
      !
      pos=index(numnam_ice_ref,"&namdyn_adv") 
      READ  ( numnam_ice_ref(pos:), namdyn_adv, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namdyn_adv in reference namelist' )
      pos=index(numnam_ice_cfg,"&namdyn_adv") 
      READ  ( numnam_ice_cfg(pos:), namdyn_adv, IOSTAT = ios, ERR = 902 )
