...
INTEGER ::  pos
      ...
      !!-------------------------------------------------------------------
      !
      pos=index(numnam_ice_ref,"&namthd_sal") 
      READ  ( numnam_ice_ref(pos:), namthd_sal, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namthd_sal in reference namelist' )
      pos=index(numnam_ice_cfg,"&namthd_sal") 
      READ  ( numnam_ice_cfg(pos:), namthd_sal, IOSTAT = ios, ERR = 902 )
