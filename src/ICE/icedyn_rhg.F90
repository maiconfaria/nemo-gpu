...
INTEGER ::  pos
      ...
      !!-------------------------------------------------------------------
      !
      pos=index(numnam_ice_ref,"&namdyn_rhg") 
      READ  ( numnam_ice_ref(pos:), namdyn_rhg, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namdyn_rhg in reference namelist' )
      pos=index(numnam_ice_cfg,"&namdyn_rhg") 
      READ  ( numnam_ice_cfg(pos:), namdyn_rhg, IOSTAT = ios, ERR = 902 )
