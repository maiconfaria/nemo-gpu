...
INTEGER ::  pos
      ...
      ENDIF
      !
      pos=index(numnat_ref,"&namtrc_ice") 
      READ  ( numnat_ref(pos:), namtrc_ice, IOSTAT = ios, ERR = 901)
 901  IF( ios /= 0 )   CALL ctl_nam ( ios , ' namtrc_ice in reference namelist ' )
      pos=index(numnat_cfg,"&namtrc_ice") 
      READ  ( numnat_cfg(pos:), namtrc_ice, IOSTAT = ios, ERR = 902 )
