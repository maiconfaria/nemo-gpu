...
INTEGER ::  pos
      ...
      pos=index(numnat_ref,"&namtrc_bc") 
      READ  ( numnat_ref(pos:), namtrc_bc, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namtrc_bc in reference namelist' )
      pos=index(numnat_cfg,"&namtrc_bc") 
      READ  ( numnat_cfg(pos:), namtrc_bc, IOSTAT = ios, ERR = 902 )
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namtrc_bc in configuration namelist' )
      IF(lwm) WRITE ( numont, namtrc_bc )

      IF ( ln_bdy ) THEN
      pos=index(numnat_ref,"&namtrc_bdy") 
         READ  ( numnat_ref(pos:), namtrc_bdy, IOSTAT = ios, ERR = 903)
903      IF( ios /= 0 )   CALL ctl_nam ( ios , 'namtrc_bdy in reference namelist' )
      ...
      pos=index(numnat_cfg,"&namtrc_bdy") 
         READ  ( numnat_cfg(pos:), namtrc_bdy, IOSTAT = ios, ERR = 904 )
