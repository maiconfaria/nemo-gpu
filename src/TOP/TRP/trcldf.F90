...
INTEGER ::  pos
      ...
      ENDIF
      !
      pos=index(numnat_ref,"&namtrc_ldf") 
      READ  ( numnat_ref(pos:), namtrc_ldf, IOSTAT = ios, ERR = 903)
903   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namtrc_ldf in reference namelist' )
      !
      pos=index(numnat_cfg,"&namtrc_ldf") 
      READ  ( numnat_cfg(pos:), namtrc_ldf, IOSTAT = ios, ERR = 904 )
