...
INTEGER ::  pos
      ...
      !!----------------------------------------------------------------------
      !
      pos=index(numnat_ref,"&namtrc_dmp") 
      READ  ( numnat_ref(pos:), namtrc_dmp, IOSTAT = ios, ERR = 909)
909   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namtrc_dmp in reference namelist' )
      pos=index(numnat_cfg,"&namtrc_dmp") 
      READ  ( numnat_cfg(pos:), namtrc_dmp, IOSTAT = ios, ERR = 910)
