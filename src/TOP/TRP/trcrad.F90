...
INTEGER ::  pos
      ...
      !!----------------------------------------------------------------------
      !
      pos=index(numnat_ref,"&namtrc_rad") 
      READ  ( numnat_ref(pos:), namtrc_rad, IOSTAT = ios, ERR = 907)
907   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namtrc_rad in reference namelist' )
      pos=index(numnat_cfg,"&namtrc_rad") 
      READ  ( numnat_cfg(pos:), namtrc_rad, IOSTAT = ios, ERR = 908 )
