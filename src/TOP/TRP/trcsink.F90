...
INTEGER ::  pos
      ...
      !!----------------------------------------------------------------------
      !
      pos=index(numnat_ref,"&namtrc_snk") 
      READ  ( numnat_ref(pos:), namtrc_snk, IOSTAT = ios, ERR = 907)
907   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namtrc_snk in reference namelist' )
      pos=index(numnat_cfg,"&namtrc_snk") 
      READ  ( numnat_cfg(pos:), namtrc_snk, IOSTAT = ios, ERR = 908 )
