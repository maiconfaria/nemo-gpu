...
CONTAINS
   ...
   SUBROUTINE trc_adv_ini
      ...
      !
      !                                !==  Namelist  ==!
      pos=index(numnat_ref,"&namtrc_adv") 
      READ  ( numnat_ref(pos:), namtrc_adv, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namtrc_adv in reference namelist' )
      pos=index(numnat_cfg,"&namtrc_adv") 
      READ  ( numnat_cfg(pos:), namtrc_adv, IOSTAT = ios, ERR = 902 )
