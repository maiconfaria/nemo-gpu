...
INTEGER ::  pos
      ...
      ENDIF
      !
      pos=index(numnat_ref,"&namtrc_dta") 
      READ  ( numnat_ref(pos:), namtrc_dta, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namtrc_dta_ini in reference namelist' )
      pos=index(numnat_cfg,"&namtrc_dta") 
      READ  ( numnat_cfg(pos:), namtrc_dta, IOSTAT = ios, ERR = 902 )
