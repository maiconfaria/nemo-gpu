...
INTEGER ::  pos
      ...
      !                             !--------------------!
      !
      pos=index(numnam_ref,"&namctl") 
      READ  ( numnam_ref(pos:), namctl, IOSTAT = ios, ERR = 901 )
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namctl in reference namelist' )
      pos=index(numnam_cfg,"&namctl") 
      READ  ( numnam_cfg(pos:), namctl, IOSTAT = ios, ERR = 902 )
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namctl in configuration namelist' )
      ...
      !                             !------------------------------------!
      !     
      pos=index(numnam_ref,"&namcfg") 
      READ  ( numnam_ref(pos:), namcfg, IOSTAT = ios, ERR = 903 )
903   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namcfg in reference namelist' )
      pos=index(numnam_cfg,"&namcfg") 
      READ  ( numnam_cfg(pos:), namcfg, IOSTAT = ios, ERR = 904 )
