...
INTEGER ::  pos
      ...
      !!----------------------------------------------------------------------
      !
      pos=index(numnam_ref,"&namdta_dyn") 
      READ  ( numnam_ref(pos:), namdta_dyn, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namdta_dyn in reference namelist' )
      pos=index(numnam_cfg,"&namdta_dyn") 
      READ  ( numnam_cfg(pos:), namdta_dyn, IOSTAT = ios, ERR = 902 )
