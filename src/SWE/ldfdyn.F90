...
CONTAINS
   ...
   SUBROUTINE ldf_dyn_init
      ...
      !!----------------------------------------------------------------------
      !
      pos=index(numnam_ref,"&namdyn_ldf") 
      READ  ( numnam_ref(pos:), namdyn_ldf, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namdyn_ldf in reference namelist' )

      pos=index(numnam_cfg,"&namdyn_ldf") 
      READ  ( numnam_cfg(pos:), namdyn_ldf, IOSTAT = ios, ERR = 902 )
