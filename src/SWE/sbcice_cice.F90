...
INTEGER ::  pos
      ...
      pos=index(numnam_ref,"&namsbc_cice") 
         READ  ( numnam_ref(pos:), namsbc_cice, IOSTAT = ios, ERR = 901)
901      IF( ios /= 0 )   CALL ctl_nam ( ios , 'namsbc_cice in reference namelist' )

      pos=index(numnam_cfg,"&namsbc_cice") 
         READ  ( numnam_cfg(pos:), namsbc_cice, IOSTAT = ios, ERR = 902 )
