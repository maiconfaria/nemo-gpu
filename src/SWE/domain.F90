...
INTEGER ::  pos
      ...
      !
      !
      pos=index(numnam_ref,"&namrun") 
      READ  ( numnam_ref(pos:), namrun, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namrun in reference namelist' )
      pos=index(numnam_cfg,"&namrun") 
      READ  ( numnam_cfg(pos:), namrun, IOSTAT = ios, ERR = 902 )
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namrun in configuration namelist' )
      ...
      pos=index(numnam_ref,"&namdom") 
      READ  ( numnam_ref(pos:), namdom, IOSTAT = ios, ERR = 903)
903   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namdom in reference namelist' )
      pos=index(numnam_cfg,"&namdom") 
      READ  ( numnam_cfg(pos:), namdom, IOSTAT = ios, ERR = 904 )
...
!set output file type for XIOS based on NEMO namelist 
      ...
      pos=index(numnam_ref,"&namnc4") 
      READ  ( numnam_ref(pos:), namnc4, IOSTAT = ios, ERR = 907)
907   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namnc4 in reference namelist' )
      pos=index(numnam_cfg,"&namnc4") 
      READ  ( numnam_cfg(pos:), namnc4, IOSTAT = ios, ERR = 908 )
