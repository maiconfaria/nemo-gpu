...
MODULE cuda_fortran
    ...
    CONTAINS
      ATTRIBUTES(global) &
      SUBROUTINE dia_hsb_kernel(surf, e3t_n, surf_ini, e3t_ini, ts, hc_loc_ini, sc_loc_ini,     &
                            & tmask, tmask_ini, zwrkv, zwrkh, zwrks, zwrk, jpi, jpj, jpk, jpt, Kmm)
            !
            INTEGER, VALUE :: jpi, jpj, jpk                                                                         ! MPI process sub-domain and stream tile
            ...
            IF ( (i .le. jpi ) .AND. (j .le. jpj) .AND. (k .le. jpk-1) ) THEN
                zwrkv(i, j, k) = ( surf(i, j) * e3t(i, j, k, Kmm) * tmask(i,j,k) - surf_ini(i, j) * e3t_ini(i, j, k) ) * tmask_ini(i, j, k)
