...
CONTAINS
   ...
   SUBROUTINE dia_hsb( kt, Kbb, Kmm )
      ...
      IF( ln_rnf_sal)   z_frc_trd_s = z_frc_trd_s + glob_sum( 'diahsb', rnf_tsc(:,:,jp_sal) * surf(:,:) )       !                    ! Add ice shelf heat & salt input
      IF( ln_isf    )   z_frc_trd_t = z_frc_trd_t &                                                                                  
                             & + glob_sum( 'diahsb', ( risf_cav_tsc(:,:,jp_tem) + risf_par_tsc(:,:,jp_tem) ) * surf(:,:) )           ! Add penetrative solar radiation
      IF( ln_traqsr )   z_frc_trd_t = z_frc_trd_t + r1_rho0_rcp * glob_sum( 'diahsb', qsr     (:,:) * surf(:,:) )                    ! Add geothermal heat flux
      ...
      & d_tsn, d_hc_loc_ini, d_sc_loc_ini, d_tmask, d_tmask_ini, d_zwrkv, d_zwrkh, d_zwrks, d_zwrk, jpi, jpj, jpk)
      !
      ...
      istat = cudaMemcpyAsync( h_ztmpv(tile_n)   ,  ztmpv    , 1  , str )
      !$cuf kernel do <<< *, *, stream=str >>>
      do ji = 1, localsize
        ztmph = ztmph + d_zwrkh(ji)
      end do
      istat = cudaMemcpyAsync( h_ztmph(tile_n)   ,  ztmph    , 1  , str )
      !$cuf kernel do <<< *, *, stream=str >>>
      do ji = 1, localsize
        ztmps = ztmps + d_zwrks(ji)
      end do
      istat = cudaMemcpyAsync( h_ztmps(tile_n)   ,  ztmps    , 1  , str )
      !$cuf kernel do <<< *, *, stream=str >>>
      do ji = 1, localsize
        ztmp  = ztmp  + d_zwrk(ji)
      end do
      istat = cudaMemcpyAsync( h_ztmp (tile_n)   ,  ztmp     , 1  , str )
